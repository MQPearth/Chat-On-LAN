...
public class PrivateChatUI extends JFrame
{
	...
	private JTextArea chatRecordTextArea = new JTextArea(); // 聊天记录
	private JTextArea inputMessageTextArea = new JTextArea();// 消息输入框
	private JScrollPane sp=null; //聊天记录滚动面板
	private JButton exitButton = new JButton("关闭私聊");// 退出按钮
	private JButton sendButton = new JButton("发送");// 发送按钮
	private JButton sendFileButton = new JButton("发送文件");// 发送按钮
	private String title; // 窗口标题
	public static HashMap<String, PrivateChatUI> privateChatUIs = new HashMap<>();// 私聊窗口
	...
	public PrivateChatUI(String title)
	{
		...
		chatRecordTextArea.setMargin(new Insets(10, 10, 10, 10));//文本与文本域的边距
		chatRecordTextArea.setEditable(false);//不可编辑
		chatRecordTextArea.setLineWrap(true);
		chatRecordTextArea.setFont(new Font("微软雅黑", 0, 16));	
		...
		inputMessageTextArea.setFont(new Font("微软雅黑", 0, 16));
