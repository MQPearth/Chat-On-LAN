...
{
	private static long recFileSize; // 将接收文件的大小
	...
	public static void sendFile(final String ip)
	{
		...
		new Thread()
		{
			...
			public void run()
			{
				...
				try
				{
					long count = 0;// 已发送数据的大小
					JLabel label = showProgress(ChatRoomUI.getInstance());
					try
					{
						socket = new Socket();
						socket.connect(new InetSocketAddress(ip, 12346), 10 * 1000);// 指定超时
						socket.setKeepAlive(true);
						System.out.println("连接到目标ip");
						dout = new DataOutputStream(socket.getOutputStream());
						File file = PrivateChatSendFileListener.file; // 获取选择的文件
